...
/*	for ( int i = 0; i < MAX_HISTORY; i++ )
	{
		if ( rgAmmoHistory[i].type )
		{
			...
			{
				wrect_t rect;
				HSPRITE *spr = gWR.GetAmmoPicFromWeapon( rgAmmoHistory[i].iId, rect );
				...
				// Draw the number
				gHUD.DrawHudNumberString( xpos - 10, ypos, xpos - 100, rgAmmoHistory[i].iCount, 255, 255, 255);
			}
			else if ( rgAmmoHistory[i].type == HISTSLOT_WEAP )
			{
				WEAPON *pw =  gWR.GetWeapon( rgAmmoHistory[i].iId );	
				if ( !pw )
					return 1;  // we don't know about the weapon yet, so don't draw anything

				int y = ScreenHeight - (AMMO_PICKUP_PICK_HEIGHT + (AMMO_PICKUP_GAP * i));
				int x = ScreenWidth - (MAX_WEAPON_NAME);
			...
			else if ( rgAmmoHistory[i].type == HISTSLOT_ITEM )
			{
				...
				SPR_Set( gHUD.GetSprite( rgAmmoHistory[i].iId ), 255, 255, 255 );
			    SPR_DrawHoles( 0, xpos, ypos, &rect );
